...
struct Node
{
    ...
    int predict(float feature_response) const { return (polarity * (feature_response - threshold)) > 0 ? POS : NEG; }
...
typedef QList<Node> Cascade;

class AdaBoost
{
public:
    AdaBoost(TrainingData &td) : td(td) {}
    Cascade train(int num_classifiers); // mostly a wrapper to call selectBestNode() and updateWeights() num_classifiers times.
...
private:
    ...
    Cascade classifier; // classifier to be trained
};
