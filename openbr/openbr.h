 ...
 * \param ignore bounding boxes with one side smaller than filter
 * \return Average detection bounding box overlap.
 */
BR_EXPORT float br_eval_detection(const char *predicted_gallery, const char *truth_gallery, const char *csv = "", bool normalize = false, int filter = -1);
