...
/*!
 * Data structure representing a single face.
 */
typedef struct sface {
    ...
    uint8_t format; /**< Flag indicating native format of the image as supplied to NIST:
                             - 0x01 = JPEG (i.e. compressed data)
                             - 0x02 = PNG (i.e. never compressed data) */
    uint8_t* data; /**< Pointer to raster scanned data. Either RGB color or intensity.
                            - If image_depth == 24 this points to  3WH bytes  RGBRGBRGB...
                            - If image_depth ==  8 this points to  WH bytes  IIIIIII */
    std::string description; /**< Single description of the image. */
} ONEFACE;

/**
 * @brief
 * Data structure representing a set of images from a single person
 * @details
...
/**
 * @brief
 * This function takes a MULTIFACE, and outputs a proprietary template for
 * enrollment.
 *
 * @details
 * The memory for the output template is allocated by the NIST test harness
 * before
 * the call i.e. the implementation shall not allocate memory for the result.
